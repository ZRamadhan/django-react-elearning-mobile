...
export default class StatusTersangkaEdit extends React.Component {
  state = {
    form: {},
    loading: false,
  }
  ...
  render(){
    ...
    return (
      <NavigationBar hideSearch disableMenu renderButton={buttonGroup} loading={this.state.loading}>
        ...
        <FormGroup title="Edit Status Tersangka" formData={formData}/>
        <Button
          title={`Edit Status Tersangka-${this.props.id}`}
          type="outline"
          containerStyle={{padding:10}}
          onPress={() => {
