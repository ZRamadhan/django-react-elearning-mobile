...
class ProsesTersangkaList extends React.Component {
  state = {
    loading: false,
  }
  ...
  render(){
    ...
    const buttonGroup = (
      <React.Fragment>
        <Button
          title="Buat Proses Tersangka"
          type="outline"
          icon={<Icon style={{fontSize:15, color:'#517fa4', padding:8}} name='create' />}
          containerStyle={{padding:10}}
          onPress={()=>this.props.navigation.navigate('prosesTSK.new')}
        />
      </React.Fragment>
    )
    ...
    return (
      <NavigationBar hideSearch renderButton={buttonGroup} loading={this.state.loading}>
        <SafeAreaView style={styles.container}>
          <Text>Ini Proses Tersangka List</Text>
        </SafeAreaView>
      </NavigationBar>
    )
  }
...
export default connect(mapStateToProps)(ProsesTersangkaList)
